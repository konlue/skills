#!/usr/bin/env bash
# Coldreview installer — 把代码自审 skill 装到各个 AI 编程工具里
# 用法: ./install.sh [--all|--project|--codex|--cc|--cursor|--opencode|--pi|--dsh|--workbuddy|--zcode|--trae|--qoder|--uninstall]
set -euo pipefail

RAW_BASE="https://raw.githubusercontent.com/konlue/skills/main/coldreview"
MARK_START="<!-- coldreview:start -->"
MARK_END="<!-- coldreview:end -->"
...
用法:
  ...
  ./install.sh --dsh           DSH:    ~/.dsh/skills/coldreview/SKILL.md
  ./install.sh --workbuddy     WorkBuddy: ~/.workbuddy/skills/coldreview/SKILL.md
  ./install.sh --zcode         Zcode:  ~/.zcode/skills/coldreview/SKILL.md
  ./install.sh --trae          Trae:   .trae/rules/coldreview.md
  ./install.sh --qoder         Qoder:  ~/.qoder/skills/coldreview/SKILL.md
  ./install.sh --all           全部平台
...
# 从文件里剥掉旧的 coldreview 段落（幂等 + 可卸载，不动文件其余内容）
strip_block() {
  local file="$1"
  [ -f "$file" ] || return 0
...
do_cc() {
  ...
  cp "$SRC" "$PWD/.claude/commands/coldreview.md"
  echo "  ✔ 安装到 .claude/commands/coldreview.md（对话里输入 /coldreview 触发）"
}

do_cursor() {
  echo "[Cursor] 项目规则"
  local file="$PWD/.cursor/rules/coldreview.mdc"
  mkdir -p "$(dirname "$file")"
  {
    echo "---"
    echo "description: 代码自审——切换成 Reviewer 角色，不改代码，只查边界条件、异常处理、性能瓶颈、安全风险"
    echo "alwaysApply: true"
    echo "---"
    echo
    body
  } > "$file"
  echo "  ✔ 安装到 $file（对话里 @coldreview 可手动引用）"
}
...
do_dsh() {
  echo "[DSH] 全局 skill"
  install_skill_dir "${DSH_HOME:-$HOME/.dsh}/skills/coldreview"
}

do_workbuddy() {
  echo "[WorkBuddy] 全局 skill"
  install_skill_dir "$HOME/.workbuddy/skills/coldreview"
}

do_zcode() {
  echo "[Zcode] 全局 skill"
  install_skill_dir "$HOME/.zcode/skills/coldreview"
}

do_trae() {
  echo "[Trae] 项目规则"
  local file="$PWD/.trae/rules/coldreview.md"
  mkdir -p "$(dirname "$file")"
  {
    echo "---"
    echo "description: 代码自审——切换成 Reviewer 角色，不改代码，只查边界条件、异常处理、性能瓶颈、安全风险"
...
do_qoder() {
  echo "[Qoder] 全局 skill"
  install_skill_dir "$HOME/.qoder/skills/coldreview"
}
...
do_uninstall() {
  ...
  for p in "$PWD/.trae/rules/coldreview.md" "$PWD/.cursor/rules/coldreview.mdc" \
           "$PWD/.claude/commands/coldreview.md" \
           "${DSH_HOME:-$HOME/.dsh}/skills/coldreview" \
           "$HOME/.workbuddy/skills/coldreview" \
           "$HOME/.zcode/skills/coldreview" \
           "$HOME/.qoder/skills/coldreview"; do
    if [ -e "$p" ]; then
      rm -rf "$p"
      echo "  ✔ 删除 $p"
    fi
