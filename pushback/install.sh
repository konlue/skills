#!/usr/bin/env bash
# Pushback installer — 把「别做应声虫」skill 装到各个 AI 编程工具里
# 用法: ./install.sh [--all|--project|--codex|--cc|--cursor|--opencode|--pi|--dsh|--workbuddy|--zcode|--trae|--qoder|--uninstall]
set -euo pipefail

RAW_BASE="https://raw.githubusercontent.com/konlue/skills/main/pushback"
MARK_START="<!-- pushback:start -->"
MARK_END="<!-- pushback:end -->"
...
用法:
  ...
  ./install.sh --dsh           DSH:    ~/.dsh/skills/pushback/SKILL.md
  ./install.sh --workbuddy     WorkBuddy: ~/.workbuddy/skills/pushback/SKILL.md
  ./install.sh --zcode         Zcode:  ~/.zcode/skills/pushback/SKILL.md
  ./install.sh --trae          Trae:   .trae/rules/pushback.md
  ./install.sh --qoder         Qoder:  ~/.qoder/skills/pushback/SKILL.md
  ./install.sh --all           全部平台
...
# 从文件里剥掉旧的 pushback 段落（幂等 + 可卸载，不动文件其余内容）
strip_block() {
  local file="$1"
  [ -f "$file" ] || return 0
...
do_cc() {
  ...
  cp "$SRC" "$PWD/.claude/commands/pushback.md"
  echo "  ✔ 安装到 .claude/commands/pushback.md（对话里输入 /pushback 触发）"
}

do_cursor() {
  echo "[Cursor] 项目规则"
  local file="$PWD/.cursor/rules/pushback.mdc"
  mkdir -p "$(dirname "$file")"
  {
    echo "---"
    echo "description: 别做应声虫——执行前检查错误前提与逻辑漏洞，不迎合、独立判断，发现问题直接指出并说明风险"
    echo "alwaysApply: true"
    echo "---"
    echo
    body
  } > "$file"
  echo "  ✔ 安装到 $file（对话里 @pushback 可手动引用）"
}
...
do_dsh() {
  echo "[DSH] 全局 skill"
  install_skill_dir "${DSH_HOME:-$HOME/.dsh}/skills/pushback"
}

do_workbuddy() {
  echo "[WorkBuddy] 全局 skill"
  install_skill_dir "$HOME/.workbuddy/skills/pushback"
}

do_zcode() {
  echo "[Zcode] 全局 skill"
  install_skill_dir "$HOME/.zcode/skills/pushback"
}

do_trae() {
  echo "[Trae] 项目规则"
  local file="$PWD/.trae/rules/pushback.md"
  mkdir -p "$(dirname "$file")"
  {
    echo "---"
    echo "description: 别做应声虫——执行前检查错误前提与逻辑漏洞，不迎合、独立判断，发现问题直接指出并说明风险"
...
do_qoder() {
  echo "[Qoder] 全局 skill"
  install_skill_dir "$HOME/.qoder/skills/pushback"
}
...
do_uninstall() {
  ...
  for p in "$PWD/.trae/rules/pushback.md" "$PWD/.cursor/rules/pushback.mdc" \
           "$PWD/.claude/commands/pushback.md" \
           "${DSH_HOME:-$HOME/.dsh}/skills/pushback" \
           "$HOME/.workbuddy/skills/pushback" \
           "$HOME/.zcode/skills/pushback" \
           "$HOME/.qoder/skills/pushback"; do
    if [ -e "$p" ]; then
      rm -rf "$p"
      echo "  ✔ 删除 $p"
    fi
